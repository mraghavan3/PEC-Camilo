...
int Broadphase::removeSameFiberCollisions(custom_vector<long long> &potentialCollisions, custom_vector<int> &stencil){

	thrust::remove_if(potentialCollisions.begin(), potentialCollisions.end(), stencil.begin(), thrust::identity<int>());

	thrust::inclusive_scan(stencil.begin(),stencil.end(), stencil.begin()); 
	return stencil.back();
}
int Broadphase::removeSameFiberCollisionsH(thrust::host_vector<long long> &potentialCollisions, thrust::host_vector<int> &stencil){

	thrust::remove_if(potentialCollisions.begin(), potentialCollisions.end(), stencil.begin(), thrust::identity<int>());

	thrust::inclusive_scan(stencil.begin(),stencil.end(), stencil.begin()); 
	return stencil[stencil.size()-1];
}
